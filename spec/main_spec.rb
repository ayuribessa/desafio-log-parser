...
describe '#open_file' do
	context " When loaded wrong file name or path " do
	  it "Print an error message " do
	    expect{ Parser.new('wrong_file.log')}.to raise_error("File not found")
	  end
  end
end
...
describe '#json_format' do
    context "when formating to json " do
      ...
      it "return a json with filename and number of lines" do
        parser = Parser.new('test.log')
        expect(parser.json_format).to eq '{"test.log":{"lines":1}}' #TODO: certificar apenas que possui o path e o n de linhas
      end
    end
  end
end
